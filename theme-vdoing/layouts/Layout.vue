...
import live2D from '@theme/components/ting-live2d.vue'
...
export default {
  ...
  watch: {
	...
	$route:{
	  handler(to){
		  let paths = ['/','/notes/','/document/','/technology/','/more/','/about/','/categories/','/tags/','/archives/']
		  if(paths.indexOf(to.path) !== -1) {
			  this.$nextTick(()=>{
				  document.querySelector('#landlord').style.display = 'block'
				  document.querySelector('#waifu').style.display='block'
			  })
		  } else {
			  this.$nextTick(()=>{
				  document.querySelector('#landlord').style.display = 'none'
